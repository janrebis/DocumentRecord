...
public class DocumentService : IDocumentService
{
    ...
    public async Task DeleteDocumentAsync(int documentId)
    {
        ...
        try
        {
            ...
            await _documentRepository.UpdateDocumentMetadataAsync(metadataId, metadata);

            _logger.LogInformation(
                "{DocumentName}: Dokument usunięty.",
                documentName);
        }
        catch (Exception ex)
        {
            ...
            catch (Exception statusUpdateException)
            {
                _logger.LogError(
                    ...
                    "{DocumentName}: Nie udało się oznaczyć usuwania jako nieudanego.",
                    documentName);
            }
    ...
    public async Task<int> UpdateDocumentAsync(int documentId, IFormFile file)
    {
        ...
        try
        {
            ...
            await _documentRepository.UpdateDocumentMetadataAsync(metadataId, existingMetadata);
    ...
    private async Task<int> AddDocumentCoreAsync(DocumentMetadata metadata, IFormFile file)
    {
        ...
        _logger.LogInformation(
            "{DocumentName}: Rozpoczęto dodawanie dokumentu.",
            documentName);
        ...
        try
        {
            ...
            var documentId = await _documentRepository.AddDocumentMetadataAsync(metadata);

            var blobKey = await _storage.AddDocumentToStorageAsync(file);

            metadata.FinishProcessing(blobKey);

            await _documentRepository.UpdateDocumentMetadataAsync(documentId, metadata);

            _logger.LogInformation(
                "{DocumentName}: Zakończono dodawanie dokumentu.",
                documentName);
        ...
        Exception originalException)
    {
        try
        {
            if (metadata.Id > 0 && metadata.ProcessingStatus == ProcessStatus.PROCESSING)
            {
