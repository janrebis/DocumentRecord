...
public class DocumentMetadata
{
    ...
    public void StartProcessing()
    {
        if (ProcessingStatus != ProcessStatus.NEW_FILE)
            throw new DocumentWrongStatusException(
                $"{DocumentName}: Dokument nie jest nowym plikiem.",
                DocumentName);
    ...
    public void FinishProcessing(string blobKey)
    {
        if (ProcessingStatus != ProcessStatus.PROCESSING)
            throw new DocumentWrongStatusException(
                $"{DocumentName}: Dokument nie jest w trakcie dodawania.",
                DocumentName);
    ...
    public void MarkToDelete()
    {
        if (ProcessingStatus != ProcessStatus.AVAILABLE)
            throw new DocumentUnavailableException(
                $"{DocumentName}: Dokument nie może zostać oznaczony do usunięcia.",
                DocumentName);
    ...
    public void EnsureMarkedToDelete()
    {
        if (ProcessingStatus != ProcessStatus.MARKED_TO_DELETE)
            throw new DocumentUnavailableException(
                $"{DocumentName}: Dokument nie jest oznaczony do usunięcia.",
                DocumentName);
    }
    ...
    public void MarkDeleted()
    {
        if (ProcessingStatus != ProcessStatus.MARKED_TO_DELETE)
            throw new DocumentWrongStatusException(
                $"{DocumentName}: Nie można oznaczyć dokumentu jako usunięty.",
                DocumentName);
    ...
    public void FailDelete()
    {
        if (ProcessingStatus != ProcessStatus.MARKED_TO_DELETE)
            throw new DocumentWrongStatusException(
                $"{DocumentName}: Nie można oznaczyć usuwania jako nieudanego.",
                DocumentName);
    ...
    public void StartUpdate()
    {
        if (ProcessingStatus != ProcessStatus.AVAILABLE)
            throw new DocumentUnavailableException(
                $"{DocumentName}: Dokument nie jest dostępny do aktualizacji.",
                DocumentName);
    ...
    public void FinishUpdate(DocumentMetadata newMetadata)
    {
        if (ProcessingStatus != ProcessStatus.PROCESSING_UPDATE)
            throw new DocumentWrongStatusException(
                $"{DocumentName}: Dokument nie jest w trakcie aktualizacji.",
                DocumentName);
